    ...
    // email(receiver, '设备 ' +devices[index].nick+' || '+ topicAry[3] + '异常 :' + jsonObj[error])
     ...
     if (topicAry[2].indexOf('onBattery') >= 0) {
      ...
      db.query('select *  from batteryRange', [],function(results, fields) {
        console.log(results)
        if(mess.body.data.value<=results[0].battery_min){
          email(receiver, '设备 ' +devices[index].nick+' || '+ topicAry[3] + '属性监控 :' + '属性已经小于'+results[0].battery_min)
        
        }else if(mess.body.data.value>=results[0].battery_max){
          email(receiver, '设备 ' +devices[index].nick+' || '+ topicAry[3] + '属性监控 :' + '属性已经大于'+results[0].battery_min)
        }
      })
     }
    }
